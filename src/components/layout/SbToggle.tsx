...
export default function SbToggle() {
  ...
  React.useEffect(() => {
    try {
      ...
      const fromAttr = document.documentElement.getAttribute("data-sb-collapsed");
      const initial =
        saved === "true" ? true : saved === "false" ? false : fromAttr === "true";
    ...
    >
      {collapsed ? (
        <svg width="18" height="18" viewBox="0 0 24 24">
          ...
          <polyline points="5 18 11 12 5 6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      ) : (
        <svg width="18" height="18" viewBox="0 0 24 24">
          <polyline points="15 18 9 12 15 6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          <polyline points="19 18 13 12 19 6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      )}
    </button>
  );
