  ...
  return (
    <header className="sticky top-0 z-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-b border-slate-200 dark:border-slate-800">
      <div className="h-14 grid grid-cols-[auto_1fr_auto] items-center gap-2 px-3">
        {/* Esquerda: menu + greeting */}
        <div className="flex items-center gap-3">
          <button className="inline-flex lg:hidden rounded-md border border-slate-300 dark:border-slate-700 px-2.5 py-1.5" onClick={openMobile} aria-label="Abrir menu">
            <svg width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M3 6h18v2H3V6m0 5h18v2H3v-2m0 5h18v2H3v-2"/></svg>
          </button>
          <button className="hidden lg:inline-flex rounded-md border border-slate-300 dark:border-slate-700 px-2.5 py-1.5" onClick={toggleCollapsed} aria-label="Alternar largura do menu">
          ...
          <div className="hidden sm:flex flex-col leading-tight ml-1">
            <span className="text-sm font-semibold text-slate-800 dark:text-slate-100">{greetingByHour()}, {name}</span>
            <span className="text-[11px] uppercase tracking-wide text-slate-500 dark:text-slate-400">{role}</span>
          </div>
        </div>
        ...
        <div className="px-2">
          <label className="relative block">
            ...
            <input type="search" placeholder="Pesquisar…" className="w-full rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 pl-9 pr-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </label>
        </div>
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <button className="rounded-md border border-slate-300 dark:border-slate-700 px-2.5 py-1.5" onClick={() => signOut({ callbackUrl: '/login' })}>
            Terminar sessão
          </button>
        </div>
      </div>
