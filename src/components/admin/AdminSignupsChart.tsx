...
export type SignupsPoint = { date: string; value: number };

export default function AdminSignupsChart({ data, height = 280 }: { data: SignupsPoint[]; height?: number }) {
  const theme = useTheme();
  const [mounted, setMounted] = React.useState(false);
  React.useEffect(() => setMounted(true), []);
  if (!mounted) return <Box sx={{ height }}><Skeleton variant="rectangular" height={height} sx={{ borderRadius: 2 }} /></Box>;

  const grid = theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)';
    ...
    <div style={{ width: '100%', height }}>
      <ResponsiveContainer>
        <AreaChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="value" stroke={theme.palette.primary.main} fill={theme.palette.primary.main} fillOpacity={0.18} strokeWidth={2} dot={{ r: 2 }} activeDot={{ r: 4 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
