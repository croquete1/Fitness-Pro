...
function buildCsv(rows: TrainerPlanTableRow[]) {
...
function downloadCsv(rows: TrainerPlanTableRow[], fileName: string) {
...
export type DashboardResponse = TrainerPlansDashboardData & {
  ok: true;
  source: 'supabase' | 'fallback';
};

export type TrainerPlansDashboardClientProps = {
  initialData: DashboardResponse;
  viewerName?: string | null;
  pageTitle?: string;
  pageSubtitle?: string | null;
  headerActions?: React.ReactNode;
  tableExtraActions?: React.ReactNode;
  renderRowActions?: (row: TrainerPlanTableRow) => React.ReactNode;
...
export default function TrainerPlansDashboardClient({
  initialData,
  viewerName,
  ...
  renderRowActions,
  emptyState,
  searchPlaceholder = 'Filtrar por plano ou cliente',
  searchLabel = 'Pesquisar',
  exportFileName = 'planos-pt',
}: TrainerPlansDashboardClientProps) {
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return base.filter((row) => matchesQuery(row, trimmed));
  ...
  const computedSubtitle = pageSubtitle
    ?? (viewerName
      ? `Resumo dos planos geridos por ${viewerName}.`
      : 'Resumo dos planos de treino ativos.');

  const hasRowActions = typeof renderRowActions === 'function';
  const empty = emptyState ?? {
    icon: '🗂️',
    title: 'Nenhum plano encontrado',
    description: 'Ajusta os filtros ou limpa a pesquisa para voltar a listar todos os planos.',
  };

  const headerActionGroup = (
    <div className="trainer-plans__headerActions">
      {headerActions}
        ...
        <header className="trainer-plans__panelHeader trainer-plans__panelHeader--table">
          ...
          <div className="trainer-plans__actions">
            ...
            <Button
              variant="secondary"
              size="sm"
              onClick={() => downloadCsv(filteredRows, exportFileName)}
        ...
        <div className="neo-table-wrapper" role="region" aria-live="polite">
          {filteredRows.length === 0 ? (
            <div className="neo-empty trainer-plans__empty">
              ...
              <p className="neo-empty__title">{empty.title}</p>
              <p className="neo-empty__description">{empty.description}</p>
            </div>
          ) : (
            <table className="neo-table trainer-plans__table">
              <thead>
                <tr>
                  ...
                  {hasRowActions && <th scope="col">Ações</th>}
                </tr>
              </thead>
              <tbody>
                {filteredRows.map((row) => {
                  ...
                  return (
                    <tr key={row.id}>
                      ...
                      {hasRowActions && <td>{actions ? <div className="trainer-plans__rowActions">{actions}</div> : null}</td>}
                    </tr>
                  );
                })}
              </tbody>
