...
const IChevron = ({ open }: { open: boolean }) => (
  <span aria-hidden style={{ transition: "transform .28s var(--ease, ease)", transform: `rotate(${open ? 90 : 0}deg)` }}>
    ▸
  </span>
);

/* Botão de afixar/recolher (substitui o hambúrguer) */
  ...
  return (
    <span
      ...
      style={{
        ...
        placeItems: "center",
        fontSize: 18,
        transform: pinned ? "rotate(0deg)" : "rotate(-20deg)",
...
export default function Sidebar({ open, onToggle, onMouseEnter, onMouseLeave }: SidebarProps) {
  ...
  // grupos, num único useMemo (evita avisos de deps)
  const groups: NavGroup[] = useMemo(() => {
    ...
    const grpPT: NavGroup = {
      ...
      icon: <ITeam />,
      items: [
        // ✅ path corrigido para evitar 404
        { label: "Clientes", href: "/dashboard/pt/clients", icon: <IUsers /> },
  ...
  // abre automaticamente o grupo com a rota ativa
  useEffect(() => {
    const g = { ...openGroups };
    groups.forEach((grp) => {
  ...
  return (
    <aside className="fp-sidebar" aria-label="Sidebar de navegação" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "space-between", marginBottom: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button
            ...
            className="pill"
            onClick={onToggle}
            aria-label={open ? "Recolher menu" : "Afixar/Expandir menu"}
            title={open ? "Recolher" : "Afixar"}
            style={{ width: 36, height: 36, display: "grid", placeItems: "center" }}
          >
            <Pin pinned={open} />
          </button>
          {open && <strong style={{ fontSize: 16, margin: 0 }}>Menu</strong>}
        </div>
        {open && <ThemeToggle />}
      </div>

      {/* Navegação com scroll próprio */}
        ...
        {groups.map((grp) => {
          ...
          return (
            <div key={grp.id}>
              ...
              <button
                type="button"
                onClick={() => setOpenGroups((s) => ({ ...s, [grp.id]: !isOpen }))}
                ...
                style={{
                  ...
                  ...(anyActive ? activeItem : null),
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {grp.icon}
                  {open && <span>{grp.label}</span>}
                </div>
