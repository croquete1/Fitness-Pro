...
export default function SessionFormClient({
  mode,
  initial,
  /** opcional: quem usa o form (admin | trainer) — disponível para futuros ajustes */
  scope,
}: {
  mode: 'create' | 'edit';
  ...
  // -------------------------------------------
  // Prefill from URL (?from|start|start_time=&to|end|end_time=)
  // on-mount only (SSR-safe via useSearchParams)
  // -------------------------------------------
  const searchParams = useSearchParams();
  const hasPrefilledRef = React.useRef(false);

  React.useEffect(() => {
    if (hasPrefilledRef.current) return; // evita repetir
    const from =
    ...
    setValues((v) => ({
      ...v,
      start_time: from ?? v.start_time,
      end_time: to ?? v.end_time,
    }));
  ...
  React.useEffect(() => {
    ...
    searchTrainers();
    searchClients();
  }, [searchTrainers, searchClients]);

  // -------------------------------------------
  // Validação de conflitos (debounced)
  // -------------------------------------------
  const [conflict, setConflict] = React.useState<{ busy: boolean; has: boolean }>(
    () => ({ busy: false, has: false })
  );
  ...
  React.useEffect(() => {
    if (
      !values.start_time ||
      !values.end_time ||
      (!values.trainer_id && !values.client_id)
    ) {
  ...
  // Validação simples: fim > início (evita regressões de horário)
...
async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
  ...
  if (conflict.has) {
    setSnack({
      open: true,
      msg: 'Conflito de agenda — ajuste o horário/PT/Cliente',
      sev: 'error',
    });
    return;
  }
  ...
  try {
    ...
    setSnack({
      open: true,
      msg: values.id ? 'Sessão atualizada ✅' : 'Sessão criada ✅',
      sev: 'success',
    });
  } catch (e: any) {
    setSnack({
      open: true,
      msg: e?.message || 'Falha ao gravar',
      sev: 'error',
    });
  } finally {
    setSaving(false);
    ...
    <Box component="form" onSubmit={onSubmit} sx={{ display: 'grid', gap: 2 }}>
      {conflict.has && (
        <Alert severity="error">
          Conflito detetado para o intervalo selecionado.
        </Alert>
      )}
