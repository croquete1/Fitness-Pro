...
import {
  Box, Stack, TextField, Autocomplete, Button, Alert, Snackbar, CircularProgress,
} from '@mui/material';
import { z } from 'zod';
...
export default function SessionFormClient({
  mode, initial, scope = 'admin',
}: { mode: 'create'|'edit'; initial?: Partial<Values>; scope?: 'admin'|'trainer' }) {
  ...
  const fetchOptions = React.useCallback((url: string, setter: (r: Option[]) => void, setLoading: (b:boolean)=>void, q?: string) => {
  ...
  const searchTrainers = React.useMemo(() => {
    ...
    return (q?: string) => {
      if (t) clearTimeout(t);
      t = window.setTimeout(() => { fetchOptions('/api/admin/trainers', setTrainers, setLoadingTr, q); }, 200);
    };
  }, [fetchOptions]);
  ...
  const searchClients = React.useMemo(() => {
    ...
    return (q?: string) => {
      if (t) clearTimeout(t);
      t = window.setTimeout(() => { fetchOptions('/api/admin/clients', setClients, setLoadingCl, q); }, 200);
    };
  }, [fetchOptions]);
  ...
  React.useEffect(() => {
    if (!values.start_time || !values.end_time || (!values.trainer_id && !values.client_id)) {
      setConflict({ busy:false, has:false }); return;
    }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      ...
      setErrors(err); setSnack({ open:true, msg:'Verifique os campos', sev:'error' }); return;
    }
    if (conflict.has) { setSnack({ open:true, msg:'Conflito de agenda — ajuste o horário/PT/Cliente', sev:'error' }); return; }

    setSaving(true);
    try {
      const base = scope === 'trainer' ? '/api/trainer/pts-schedule' : '/api/admin/pts-schedule';
      const url = values.id ? `${base}/${values.id}` : base;
  ...
  return (
    <Box component="form" onSubmit={onSubmit} sx={{ display:'grid', gap: 2 }}>
      {conflict.has && <Alert severity="error">Conflito detectado para o intervalo selecionado.</Alert>}
      ...
      <Autocomplete<Option>
        ...
        renderInput={(p) => (
          <TextField {...p} label="PT" required error={!!errors.trainer_id} helperText={errors.trainer_id || ' '} />
        )}
      />
      ...
      <Autocomplete<Option>
        ...
        renderInput={(p) => (
          <TextField {...p} label="Cliente" required error={!!errors.client_id} helperText={errors.client_id || ' '} />
        )}
      />
      ...
      <TextField select label="Estado" value={values.status} onChange={(e)=>setField('status', e.target.value as Values['status'])}>
