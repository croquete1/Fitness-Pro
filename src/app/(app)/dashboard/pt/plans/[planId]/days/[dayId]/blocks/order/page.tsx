...
import {
  Box, Paper, List, ListItem, ListItemIcon, ListItemText, IconButton,
  Stack, Button, Snackbar, Alert, Typography, CircularProgress,
} from '@mui/material';
...
type Item = { id: string; title: string | null; order_index: number | null };
...
export default function OrderBlocksPage() {
  ...
  const onKeyReorder = (i: number) => (e: React.KeyboardEvent<HTMLLIElement>) => {
    ...
    else if (e.key === 'ArrowDown') { e.preventDefault(); move(i, Math.min(items.length - 1, i + 1)); }
  };
  ...
  const saveOrder = async () => {
    ...
    setSaving(true);
    try {
      // 🔁 endpoint de reorder mais simples: envia só a lista de ids na ordem final
    ...
    <Box sx={{ p: 2 }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          <Button startIcon={<Replay />} variant="outlined" onClick={resetOrder} disabled={loading || saving}>Repor</Button>
      ...
      <Paper variant="outlined">
        ...
        {!loading && items.length === 0 && (
          <Box sx={{ p: 2 }}><Typography color="text.secondary">Sem blocos para ordenar.</Typography></Box>
        )}
          ...
          <List disablePadding sx={{ p: 1, display: 'grid', gap: 0.5 }}>
            {items.map((it, i) => (
              <ListItem
                ...
                sx={{
                  border: 1, borderColor: 'divider', borderRadius: 1.5, px: 1,
                  '&:focus-visible': (t) => ({ outline: 'none', boxShadow: `0 0 0 3px ${t.palette.primary.main}40` }),
                }}
                secondaryAction={
                  <Stack direction="row" spacing={0.5}>
                    <IconButton size="small" aria-label="Mover para cima" onClick={() => move(i, Math.max(0, i - 1))} disabled={i === 0}>
                      <ArrowUpward fontSize="inherit" />
                    </IconButton>
                    <IconButton size="small" aria-label="Mover para baixo" onClick={() => move(i, Math.min(items.length - 1, i + 1))} disabled={i === items.length - 1}>
              ...
              >
                <ListItemIcon sx={{ minWidth: 32, color: 'text.secondary' }}><DragIndicator /></ListItemIcon>
                <ListItemText
