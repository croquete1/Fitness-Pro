    ...
    <Box sx={{ minHeight:'100dvh', display:'grid', placeItems:'center', p:3 }}>
      <Paper sx={{ p:3, width:'100%', maxWidth:520, borderRadius:4 }}>
        ...
        <form onSubmit={onSubmit}>
          <Stack spacing={2}>
            <TextField label="Nome" value={form.name} onChange={(e)=>setForm(s=>({...s,name:e.target.value}))}/>
            <TextField label="Email *" type="email" value={form.email} onChange={(e)=>setForm(s=>({...s,email:e.target.value}))} required/>
            <TextField label="Palavra-passe *" type="password" value={form.password} onChange={(e)=>setForm(s=>({...s,password:e.target.value}))} required inputProps={{minLength:6}}/>
            <Button type="submit" variant="contained" disabled={loading}>{loading?'A criar…':'Criar conta'}</Button>
